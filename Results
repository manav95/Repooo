...
df=pd.read_csv(os.getcwd() + "/video-framework/Results/Extracted_frames/result.csv",index_col=0)
...
os.chdir(os.getcwd() + "/video-framework/Results/output")
...
df2=df2.sort_values("frame_name")
df2.reset_index()
...
ax4 = df4.plot(kind="area",x=df2["Time"],subplots=True, figsize=(6, 6),ylim=[0,1]).get_figure()
ax4.savefig('plot.png')